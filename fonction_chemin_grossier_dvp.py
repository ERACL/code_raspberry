...
def fonction_chemin_grossier(depart, arrivee, Map, angle_ini, angle_fin):
	...
	#Les deux goutes d'huiles se rependent dans la Map. Lorsqu'elles se renctontrent, stop vaut True et on sort de la boucle for.
		...
		M = []
		for k in L1:
			if (k[0]>0) and (Map[k[0]-1][k[1]]<=0) : #Si la position x (k[0]) du point est <0 alors on peut regarger en k[0]-1. On utilise ici une subtilite de Python : si la premiere condition n'est pas respectee alors la deuxieme n'est pas regardee (si elle l'etait cela entrainerait une erreur dans notre cas).
				M.append((k[0]-1,k[1]))
				if Map[k[0]-1][k[1]]==0 :
	...
	for k in range(compteur_n+1,1):
		...
		if (Map[point[0]-1][point[1]-1]==k):
			...
			continue
	liste_point_envers.append(point)
	longueur_liste_envers = len(liste_point_envers)
	for k in range(len(liste_point_envers)):
		liste_point.append(longueur_liste_envers-k-1)
